...
`R` has interpreted the observation number, the rat ID, and age (in days) of the rat at each observation as integers; the treatment label is a character; and the response is a double (a real number). These decisions are acceptable with one exception. Specifically, I would say that the rat ID is not a true integer, e.g. rat \# 5 is not "greater" than rat \# 3, and should instead be considered a *character*, that is, a label. Re-read in the data using the following chunk, replacing the `<XXX>` with the appropriate value. 
...
For your very last task, create your own plot in the chunk below, trying to include as many of the following characteristics as possible:
